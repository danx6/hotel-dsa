...
def search_res(res_id):
    for i in range(len(reservations)):
        if reservations[i]["id"] == res_id: return i
    return -1
...
def c_menu():
    while True:
        ...
        if c == '1':
            cid = input("ID: ").strip()
            if cid in customers: print("Error: Exists."); continue
            n, p, e = input("Name: ").strip(), input("Phone: ").strip(), input("Email: ").strip()
            if not (cid and n and p and e): print("Error: Empty field."); continue
            customers[cid] = {"name": n, "phone": p, "email": e}
            print("Customer added.")
        elif c == '2':
            for cid, d in customers.items(): print(f"{cid} | {d['name']} | {d['phone']} | {d['email']}")
        elif c == '3':
            cid = input("ID: ").strip()
            print(customers.get(cid, "Error: Not found."))
        elif c == '4':
            cid = input("ID: ").strip()
            if cid in customers:
            ...
            for cid, d in sort_cust(): print(f"{cid} | {d['name']} | {d['phone']} | {d['email']}")
        elif c == '7': break
...
def r_menu():
    while True:
        c = input("\n1.View Rooms 2.Back\nSelect: ")
        if c == '1':
            for r in rooms: print(f"Room {r['no']} | {r['type']} | Available: {r['avail']}")
        elif c == '2': break
        else: print("Invalid choice.")
...
def res_menu():
    while True:
        ...
        if c == '1':
            ...
            if search_res(rid) != -1: print("Error: Exists."); continue
            cid = input("Cust ID: ").strip()
            if cid not in customers: print("Error: Customer not found."); continue
            rno = input("Room No: ").strip()
            idx = get_room(rno)
            if idx == -1 or not rooms[idx]["avail"]: print("Error: Room invalid or unavailable."); continue
            ...
            for r in reservations: print(f"{r['id']} | Cust: {r['cid']} | Room: {r['rno']} | In: {r['in']} | Out: {r['out']}")
        elif c == '3':
            rid = input("Res ID: ").strip()
            idx = search_res(rid)
            print(reservations[idx] if idx != -1 else "Error: Not found.")
        elif c == '4':
            rid = input("Res ID: ").strip()
            idx = search_res(rid)
            if idx != -1:
            ...
            idx = search_res(rid)
            if idx != -1:
                rooms[get_room(reservations[idx]["rno"])]["avail"] = True
...
def q_menu():
    while True:
        ...
        if c == '1':
            ...
            try: print(f"Now serving: {waiting_queue.popleft()}")
            except IndexError: print("Queue is empty.")
        elif c == '3':
            if not waiting_queue: print("Queue is empty.")
            for i, n in enumerate(waiting_queue, 1): print(f"{i}. {n}")
        elif c == '4': break
        else: print("Invalid choice.")
